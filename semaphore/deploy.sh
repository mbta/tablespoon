...
echo "Retrieving task definition parameters from ${awsenv}-template."
template_task_def="$(aws ecs describe-task-definition --task-definition "${awsenv}-template")"
...
new_task_def="$(aws ecs describe-task-definition --task-definition "${awsenv}")"
new_revision="$(echo "${new_task_def}" | jq -r '.taskDefinition.revision')"

# redeploy the cluster
echo "Updating service ${awsenv} to use task definition ${new_revision}..."
aws ecs update-service --cluster="${APP}" --service="${awsenv}" --task-definition "${awsenv}:${new_revision}"

# monitor the cluster for status
deployment_finished=false
while [ "$deployment_finished" = "false" ]; do
  # get the service details
  service_status="$(aws ecs describe-services --cluster="${APP}" --services="${awsenv}")"
...
while [ "$teardown_finished" = "false" ]; do
  ...
  old_deployments="$(echo "${service_status}" | jq -r --compact-output '.services[0].deployments[] | select(.status == "ACTIVE")')"
  total_tasks=0
